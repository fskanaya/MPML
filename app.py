...
marital_status = st.radio('Status perkawinan', ['Belum Menikah', 'Menikah'])
occupation = st.radio('Pekerjaan', ['Student', 'Employee', 'Self Employed'])
monthly_income = st.number_input('Penghasilan bulanan', min_value=0, step=1)
educational_qualifications = st.radio('Kualifikasi pendidikan', ['Under Graduate', 'Graduate', 'Post Graduate'])
family_size = st.number_input('Jumlah keluarga', min_value=1, max_value=20, step=1)
latitude = st.number_input('Latitude', format="%.6f")
longitude = st.number_input('Longitude', format="%.6f")
pin_code = st.number_input('KOde pos', min_value=0, step=1)
